     ...
     axios({
      ...
      .then((invoiceRes) => {
        if(invoiceRes.data !== null || undefined){
          const invoice = invoiceRes.data.find(invoice=>invoice.invoiceId == '18-ORGONE' && invoice.version == row.version);
          ...
          let isCreatedBySame;
          let isValidate;
          if(new Date(invoice.invoiceDate).getTime() == new Date(blockchain.InvoiceDate).getTime()){
          ...
          if(new Date(invoice.createdDate).getTime() == new Date(blockchain.CreatedDate).getTime()){
